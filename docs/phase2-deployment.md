...
* **Infrastructure Right-Sizing:** Always cross-reference SIEM minimum requirements with cloud instance specs before deployment to avoid `OOMKill` crashes.
* **Firewall Logic:** Double-check specific port entries (e.g., 443 vs 433) in GCP VPC rules to ensure immediate dashboard access.
* **Automation:** Establishing a local Git workflow early saves significant time when documenting complex troubleshooting steps.